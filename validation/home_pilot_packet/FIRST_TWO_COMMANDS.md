...
```powershell
Expand-Archive -LiteralPath "$HOME\Downloads\MOSEQ_LEGACY_HOME_PILOT_2026-07-29.zip" -DestinationPath "$HOME\Downloads"
```

```powershell
powershell.exe -NoProfile -ExecutionPolicy Bypass -File "$HOME\Downloads\MOSEQ_LEGACY_HOME_PILOT_2026-07-29\00_export_wsl_backup.ps1" -OutputDestination "$HOME\Documents\MoSeq2-WSL-Backups" -DistributionName "Ubuntu-22.04"
```
